...
export class WindowModalComponent implements OnInit {
  ...
  public metadataTotal: number;
  public metadataPage: number;
  ...
  changePage(pageEvent: PageEvent){
    this.loadItems(pageEvent.pageIndex + 1, PAGESIZE);
  }

  loadItems(page: number, pagesize: number){
    this.productsService.getListProducts(page, pagesize).subscribe(({ docs, metadata })=>{
      if (docs.length > 0) {
        this.metadataTotal = metadata[0].total;
        docs = docs.map( x =>{ 
  ...
  async loadData(): Promise<void> {
    ...
    this.success = false;
    resp1 = await this.productsService.getListProducts(PAGESIZE, PAGE);
    resp2 = await this.clientsService.listarClientes();

    if (resp1.ok && resp2.ok) {
      //console.log(resp2)
      // Get the products and clients
      this.products = this.formatProduct(resp1.data[0].docs);
